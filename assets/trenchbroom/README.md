...
After that, opening `assets\maps\lqdm1.map` needs no dialog at all: the file
begins with
...
**An imported map has to say `SFPS` too, and that is not cosmetic.** `lqdm1`
comes from LibreQuake, and what LibreQuake wrote at the top of it was
`// Game: Quake`. A map that says so opens against Quake's configuration: the
material browser looks for Quake WADs instead of `assets\sprites\`, `SFPS.fgd`
is never loaded, and every entity the importer placed — `monster_spawner_*`,
`info_altar`, `info_ward_ground`, `info_ward_air`, `monster_maw` — becomes an
unknown classname drawn as a grey box with no property sheet. The map would open
and be uneditable in the only editor this project has.
`assets/maps/import-librequake.py` writes `// Game: SFPS` for that reason;
converting the geometry and the entities is not the whole conversion.

**The other half is that every classname has to be in the `.fgd`.** Three of
lqdm1's were not: `light_flame_small_white` (×16), `light_flame_large_yellow`
(×3) and `ambient_drip` (×2), all Quake entities the engine steps over —
`level.c` matches `light` with `txt_eq`, exact equality, and its comment
predicted this: the lamp family "which the imported maps have not carried yet
and would the first time one did". The importer maps the two flames to `light`
and drops the sound, so the room now lights with the 32 sources its author
placed rather than the 13 that happened to be spelled `light`. Every classname
in the shipped map is defined in `SFPS.fgd`.
...
```text
.\build\mapview.exe lqdm1
```

Leave it open beside the editor. It watches `assets\maps\lqdm1.map` and
...
```text
maps\lqdm1.map               602078 601776  0%
```
...
`atrium` used to be linked in as the second stage, and it is worth knowing what
that looked like because the route is unchanged: `assets\levels.txt` gave
`arena` a `next atrium`, and atrium's own worldspawn carried `next vault`.
Nothing else had to change. The map itself is gone -- the game stopped shipping
levels it could not reach, and a `.map` kept alive to demonstrate a mechanism
was one of them -- so `arena` names `vault` directly now. Writing a brush
level's name into any `next` still puts it on the chain.
...
| `info_altar` | yes — where a cleared wave's reward lands, if `assets/loot.txt` says `at altar`. Place none and the reward falls at the player's feet as it always did; place several and the nearest wins |
| Procedural materials previewed in the editor | yes — generated by `build\matdump.exe`, see below |
...
and are what a hand-authored `.map` textures its walls and its door with.

The other materials — `pbrick`, `ptile`, `ppanel`, `pwood`, `phex`, `pmarble`,
`prust`, `pgrid`, `plava`, `pwindow` — are **recipes**, not images.
`assets/textures.txt` describes them and the fragment shader computes them per
pixel, so there is no file to show and they used to appear as missing. Generate
previews for them once:

```text
.\build.ps1 -Tool matdump
```

`tools/matdump.c` creates a real GL context, draws each recipe through the real
shader in `RD_SWATCH` — the mode that exists to show a material with no lighting,
no fog and no dither — and reads the frame back. What lands in the file is what
the game draws, because the thing that draws the game drew it.

Run it again after editing a `proc` recipe or the shader behind it. The previews
are a build product, not tracked art: matdump writes `assets/sprites/.gitignore`
listing exactly what it produced, so a stale committed copy cannot exist. Before
you run it the editor behaves as it always did — those materials show as missing,
and typing the name onto a face is still correct in the game.

### Why they live in `assets/sprites` and why the bake ignores them

A face carries its material as a **file name**, so a preview has to be named
`pwindow.png` and has to sit in the directory the browser reads. That rules out
the `_`-prefix escape `bake.ps1` offers for files that are not sprites: a preview
called `_pwindow` would put `_pwindow` on the face and resolve to nothing in game.

So `bake.ps1` skips them by name instead. It reads `assets/textures.txt`, and a
material whose recipe carries `proc` is one the game never fetches pixels for —
`tex.c`'s `mat_make` calls `tex_make` only when there is no `proc`. That is the
same question `matdump.c` asks to decide what to write, so the two cannot drift
apart. The build prints the count it skipped rather than skipping in silence;
at 256×256 apiece these are 1.9MB, which is larger than the whole floppy budget.

### Size and scale of a preview

256 pixels over **2 UV units**, and a UV unit is two metres — sector walls carry
`LEVEL_UV` = 0.5 uv per metre, and a brush face at the default face scale of 0.5
works out to `32/(0.5*128)` = 0.5 as well. That is what the scale below means by
matching the density the rest of the game uses.

The two numbers satisfy one equation:

```text
metres the image depicts       = PREVIEW_UNITS / 0.5      = 4
metres the editor draws it over = PREVIEW_PX * 0.5 / 32   = 4
```

so a face wearing a preview in the 3D view shows the pattern at the size the game
will show it. Get that wrong and nothing looks broken — the material is simply a
different size in the editor than in play, and you find out after building a wall.
