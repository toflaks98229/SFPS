 ...
 * look to answer "what would porting this cost". Three functions today. The
 ...
 * All three used to be Win32 calls sitting in the middle of files that had no
 * other reason to know what OS they were on, and each one alone was enough to
 * pin its whole translation unit to Windows.
 *
 ...
 * 한 군데로 만들기 위해 존재합니다. 오늘 기준 함수 세 개입니다. 한 호스트에 대한 구현은
 ...
 * 찾고 그것이 그 뒤로 바뀌었는지 알아야 하는 에셋 로더. 셋 다 자신이 어떤 OS 위에 있는지 알
 * 다른 이유가 없는 파일 한가운데 놓인 Win32 호출이었고, 각각 하나만으로도 번역 단위 전체를
 * Windows에 묶어 두기에 충분했습니다.
 *
