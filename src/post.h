...
/* post_in_world_pass moved to diag.c as ::diag_pass_in_world. The flag was
   only ever read by diag.h's pass guards, and keeping it here made every
   module that wanted to CHECK the boundary include this header to ask --
   fx.c and weaponview.c took nothing else from it. post.c announces the
   transitions through DIAG_PASS_WORLD/DIAG_PASS_UI now.
   post_in_world_pass는 ::diag_pass_in_world라는 이름으로 diag.c로 옮겨 갔습니다. 그
   플래그를 읽던 것은 diag.h의 패스 가드뿐이었고, 이곳에 두면 경계를 *검사*하려는 모든
   모듈이 묻기 위해 이 헤더를 포함해야 했습니다. fx.c와 weaponview.c는 여기서 다른
   무엇도 가져가지 않았습니다. 이제 post.c가 DIAG_PASS_WORLD/DIAG_PASS_UI로 전이를
   알립니다. */
