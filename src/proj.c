...
void proj_boom_fx(Pools *pl, v3 at, v3 normal, float radius, FxTint tint) {
    ...
    fx_spawn_tinted(pl, "blastflash", at, normal, 1.0f, tint);
    ...
    /* THE SECOND THE BLAST SPENDS AS A CLOUD. Everything above this line
       ...
       *폭발이 구름으로 보내는 그 1초.* 이 줄 위의 모든 것은 가산입니다. 돔과 코어와
       ...
       벽을 따라 기울지 않습니다. */
    fx_spawn_tinted(pl, "blastfire", at, v3f(0, 1, 0), 1.0f, tint);
    ...
    /* THE SMOKE AND THE DEBRIS ARE NOT TINTED, and the split is the whole rule:
       ...
       burned -- `blastwave` above is the third, because dust lifted off a floor
       is the floor's colour and not the flash's. Smoke is grey because it is
       smoke, and a chip of floor thrown by an axe is the same chip thrown by a
       grenade; painting them the colour of the flash would say the explosion
       changed what the room is made of.
       *연기와 파편에는 색조를 넣지 않으며*, 그 구분이 규칙의 전부입니다. 색조가 들어가는
       겹은 *타고 있는* 것이고, 이 둘은 *탄* 것입니다. 위의 `blastwave`가 세 번째인데, 바닥에서
       들린 먼지는 섬광의 색이 아니라 그 바닥의 색이기 때문입니다. 연기가 회색인 이유는 그것이
       연기이기 때문이고, 도끼가 튀긴 바닥 조각은 유탄이 튀긴 바로 그 조각입니다. 이것들을
       섬광의 색으로 칠하는 것은 폭발이 방의 재질을 바꿨다고 말하는 셈입니다. */
    ...
    /* WHAT IS STILL BURNING A SECOND LATER. Every layer above this one is
       ...
       계속 폭발 지점을 찾아냅니다. */
    fx_spawn_tinted(pl, "blastember", at, normal, 1.0f, tint);
